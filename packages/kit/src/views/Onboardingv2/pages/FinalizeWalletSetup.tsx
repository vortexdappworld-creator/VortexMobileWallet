  ...
  const handleLetsGo = useCallback(async () => {
    ...
    if (createdWallet) {
      try {
        ...
        // Vortex demo: post-onboarding "Have a referral code?" prompt is
        // disabled. We still await checkResp so the referral status request
        // resolves (or times out) before continuing, but we never open the
        // dialog — the flow falls through to proceedToWallet() below.
        void checkResp;
      } catch {
