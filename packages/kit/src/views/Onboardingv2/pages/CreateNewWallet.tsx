...
function CreateNewWallet() {
  ...
  return (
    <OnboardingPage>
      ...
      >
        {/* Vortex demo: original "Continue with email" sidebar replaced with
            a non-custodial / BSC-DeFi pitch (no SSO, no yield claims). */}
        <OnboardingSidebar $md={{ pt: '$5' }}>
          {md ? null : <OnboardingIconBadge icon="Shield2CheckOutline" />}
          <YStack gap="$6">
            <SizableText size="$headingMd">
              A non-custodial wallet built for BSC
            </SizableText>
            {[
              {
                icon: 'KeyOutline' as const,
              ...
              },
              {
                ...
                title: 'Built for BSC DeFi',
                description:
                  'Native PancakeSwap routing and BNB Chain support out of the box.',
              },
              {
                icon: 'LockOutline' as const,
                title: 'Encrypted on-device',
                description:
                  'All keys stored encrypted locally; nothing synced anywhere.',
              },
            ].map((item) => (
              <XStack key={item.title} gap="$5" alignItems="flex-start">
                {md ? (
                  <Icon
                    name={item.icon}
                    color="$iconSubdued"
        ...
        >
          {/* Vortex demo: "Continue with Google" and "Continue with Apple"
              social-login buttons removed; only seed-phrase flow is offered. */}
          {isWebKeylessSidePanelMode ? null : (
            <>
              {/* Vortex demo: OR divider removed (no SSO buttons above it). */}
              <Button
