...
// Vortex fork — coins mirrored from the web frontend's swap catalog
// (`frontend/src/config/swap.ts`). BNB native + USDT come back from OneKey's
// own defaults; the rest get post-injected into the home Tokens list below.
const VORTEX_BSC_NETWORK = 'evm--56';
const VORTEX_BSC_INJECT: ReadonlyArray<{
  address: string;
  symbol: string;
  name: string;
  decimals: number;
...
function TokenListBlock({
  ...
  const { result: homeDefaultTokenMap } = usePromiseResult(async () => {
    ...
    // Vortex fork — drop OneKey's BSC defaults (USDC/DAI etc.) and force-add
    // the swap-catalog from vortexdapp.world so the wallet's Tokens list
    // mirrors the website. The matching tokens are injected into the fetched
    // token list below with `isAggregateToken: true` so this map's filter
    // (filter requires aggregate-or-native) actually keeps them visible at
    // zero balance, just like OneKey's own USDT/USDC defaults work.
    const VORTEX_BSC_SYMBOLS = ['BNB', 'USDT', 'BTCB', 'ETH', 'SOL', 'XRP', 'DOGE'];
    const filtered: typeof r = {};
    for (const [k, v] of Object.entries(r)) {
      if (v.networkId !== VORTEX_BSC_NETWORK) {
        filtered[k] = v;
      }
    }
    VORTEX_BSC_SYMBOLS.forEach((symbol, i) => {
      filtered[
        buildHomeDefaultTokenMapKey({
          networkId: VORTEX_BSC_NETWORK,
          symbol,
        })
      ] = {
        symbol,
    ...
    async () => {
      ...
      try {
        ...
        if (mergeDeriveAddressData) {
          ...
          // Vortex fork — inject the swap-catalog tokens into the main Tokens
          // list when looking at BSC. fetchAccountTokens already returns BNB
          // (native) and USDT (OneKey default), but the server sends BTCB/ETH/
          // SOL/XRP/DOGE to smallBalanceTokens at zero balance, which hides
          // them. We mirror the way OneKey keeps USDT visible: pull live price
          // via fetchTokensDetails and append the entries to r.tokens.data
          // tagged isAggregateToken so the homeDefaultTokenMap filter keeps
          // them at zero balance — same path as USDT/USDC do organically.
          if (network.id === VORTEX_BSC_NETWORK && account?.id) {
            try {
              const missing = VORTEX_BSC_INJECT.filter(
                (t) =>
                  !r.tokens.data.some(
                    (d) =>
                      d.address?.toLowerCase() === t.address.toLowerCase() ||
                      (d.commonSymbol ?? d.symbol)?.toUpperCase() ===
              ...
              if (missing.length > 0) {
                const details =
                  await backgroundApiProxy.serviceToken.fetchTokensDetails({
                    accountId,
                    networkId: network.id,
                    contractList: missing.map((t) => t.address),
                  });
                for (const t of missing) {
                  const $key = `${VORTEX_BSC_NETWORK}_${t.address}`;
                  const detail = details.find(
                    (d) =>
                      d.info.address?.toLowerCase() ===
                      t.address.toLowerCase(),
                  );
                  const item: IAccountToken = {
                    $key,
                    address: t.address,
                    symbol: t.symbol,
                    name: t.name,
                    ...
                    logoURI:
                      detail?.info.logoURI ||
                      `https://cdn.jsdelivr.net/gh/trustwallet/assets@master/blockchains/smartchain/assets/${t.address}/logo.png`,
                    accountId,
                    networkName: network.name,
                  };
                  r.tokens.data.push(item);
                  r.tokens.map[$key] = {
                    balance: detail?.balance ?? '0',
                    balanceParsed: detail?.balanceParsed ?? '0',
                    fiatValue: detail?.fiatValue ?? '0',
                    price: detail?.price ?? 0,
                    price24h: detail?.price24h ?? 0,
                    currency: detail?.currency,
                  };
                }
              }
            } catch (e) {
              console.warn('[Vortex] inject BSC tokens failed', e);
            }
          }
