...
export function HomePageView({
  ...
  // Vortex demo: force BSC as the default network on first session boot once
  // a wallet exists. Ref-guarded so a user manually switching network within
  // the same session isn't fought; next app launch resets to BSC again.
  const VORTEX_DEFAULT_BSC_DONE = useRef(false);
  const accountSelectorActions = useAccountSelectorActions();
  useEffect(() => {
    if (VORTEX_DEFAULT_BSC_DONE.current) return;
    if (!wallet?.id) return;
    VORTEX_DEFAULT_BSC_DONE.current = true;
    void accountSelectorActions.current.updateSelectedAccountNetwork({
      num: 0,
      networkId: 'evm--56',
    });
  }, [wallet?.id, accountSelectorActions]);
  ...
  const tabConfigs = useMemo(() => {
    // Vortex demo: customer-requested order — Spot, History, DeFi, NFT.
    return [
      {
