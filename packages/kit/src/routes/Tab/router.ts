...
import { vortexRouters } from './Vortex/router';

// VORTEX DEMO: use SF Symbols (native iOS icons) for the bottom tab bar.
// The original SVG `require()` icons need a build-time xcassets generation
// step we can't run in our partial-rebuild pipeline — SF Symbols are
// guaranteed to render natively without any asset shipping.
const nativeTabIcons = {
  wallet: (): INativeTabBarIcon => ({ sfSymbol: 'creditcard.fill' }),
  swap: (): INativeTabBarIcon => ({ sfSymbol: 'arrow.left.arrow.right' }),
  discover: (): INativeTabBarIcon => ({ sfSymbol: 'safari' }),
  market: (): INativeTabBarIcon => ({ sfSymbol: 'chart.line.uptrend.xyaxis' }),
  perp: (): INativeTabBarIcon => ({ sfSymbol: 'chart.bar.xaxis' }),
  earn: (): INativeTabBarIcon => ({ sfSymbol: 'dollarsign.circle.fill' }),
  vortex: (): INativeTabBarIcon => ({ sfSymbol: 'diamond.fill' }),
  developer: (): INativeTabBarIcon => ({ sfSymbol: 'chevron.left.forwardslash.chevron.right' }),
};
  ...
  return useMemo(() => {
    const tabs = [
      ...
      shouldShowMarketTab
        ...
        : undefined,
      // Vortex demo: customer-requested bottom tab order on native is
      // Wallet -> Vortex -> Swap -> Discover -> Perps. Vortex is placed
      // here (right after Home/Wallet), and Perp tabs are moved to the
      // end below (after the Discover entries).
      {
        name: ETabRoutes.Vortex,
        tabBarIcon: (focused?: boolean) =>
          focused ? 'DiamondSolid' : 'DiamondOutline',
        nativeTabBarIcon: nativeTabIcons.vortex,
        translationId: ETranslations.global_earn,
        tabBarLabelText: 'Vortex',
        freezeOnBlur: Boolean(params?.freezeOnBlur),
        rewrite: '/vortex',
        exact: true,
        children: vortexRouters,
        trackId: 'global-vortex',
      },
      {
      ...
      // Vortex demo: Perp tabs moved to the end so the visible native order
      // becomes Wallet, Vortex, Swap, Discover, Perps.
      {
