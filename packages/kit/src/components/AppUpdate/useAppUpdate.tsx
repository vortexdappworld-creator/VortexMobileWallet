...
export const useAppUpdateInfo = (isFullModal = false, autoCheck = false) => {
  ...
  return useMemo(() => {
    const { fileType } = isNeedUpdate({
      latestVersion: appUpdateInfo.latestVersion,
      jsBundleVersion: appUpdateInfo.jsBundleVersion,
      status: appUpdateInfo.status,
    });
    return {
      // Vortex fork — OneKey's update server reports its own (higher) version,
      // which triggered a misleading "Vortex just got better!" prompt and an
      // in-app update reminder. Force `isNeedUpdate=false` everywhere so no
      // banner/dialog can surface until we wire up our own release channel.
      isNeedUpdate: false,
      updateFileType: fileType,
      data: appUpdateInfo,
      onUpdateAction,
