...
export function AppUpdateForeground() {
  // Vortex fork — OneKey's remote update server reports its own (higher) version
  // number, which triggers a misleading "Vortex just got better!" dialog. Disable
  // the foreground update flow entirely until we wire up our own release channel.
  useAppUpdateForegroundEffects(false);
  return null;
}
