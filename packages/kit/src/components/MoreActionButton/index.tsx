...
function MoreActionContentHeader({
  ...
  const items = useMemo(() => {
    return [
      ...popupMenu,
      // Vortex demo: headphones / "Contact us" header button removed.
      firstActionItem,
    ];
  }, [intl, popupMenu, firstActionItem]);
      ...
      >
        <XStack gap="$1" ai="center" jc="center">
          ...
          <SizableText size="$bodyMdMedium" color="$textSubdued">
            {`${intl.formatMessage({ id: ETranslations.global_about })} Vortex`}
          </SizableText>
        </XStack>
        <XStack gap="$1" ai="center" jc="center">
...
function MoreActionGeneralGrid() {
  ...
  const items = useMemo(() => {
    return [
      ...
      // Vortex demo: "Prime" entry removed.
      false ? { title: 'Prime', icon: 'PrimeOutline' as const, onPress: handlePrime } : undefined,
...
const MoreActionWalletGrid = () => {
  ...
  const items = useMemo(() => {
    return [
      ...
      // Vortex demo: Prime-gated "Bulk Copy Addresses" and "Bulk Send"
      // entries removed from MoreActionButton.
      undefined,
      undefined,
    ].filter(Boolean);
  }, [
...
const MoreActionMoreGrid = () => {
  ...
  const items = useMemo(() => {
    return [
      ...
      // Vortex demo: "Refer a friend" and "Redeem" entries removed.
    ];
  }, [
  ...
  return (
    <YStack flex={1}>
      <ScrollView overflow="scroll" flex={1}>
        ...
        {/* Vortex demo: OneKey ID header and "Connect hardware wallet"
            entry removed from the More menu. */}
        {null}
        {null}
        <MoreActionDivider />
        ...
        {/* Vortex demo: entire "More" section (Support / Referral / Redeem) removed. */}
      </ScrollView>
