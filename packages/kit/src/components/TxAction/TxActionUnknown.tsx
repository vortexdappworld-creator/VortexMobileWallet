...
function TxActionUnknownListView(props: ITxActionProps) {
  ...
  // Vortex demo: when the unknown call targets the Vortex contract, show a
  // branded title instead of the generic "Contract Interaction".
  const isVortexTo =
    typeof unknownTo === 'string' &&
    unknownTo.toLowerCase() ===
      '0x729f8f6a6970cfc7acae555236a20751c1501f39';

  const title = isVortexTo
...
function TxActionUnknownDetailView(props: ITxActionProps) {
  ...
  // Vortex demo: brand the detail view when the target is the Vortex
  // contract.
  const isVortexTo =
    typeof unknownTo === 'string' &&
    unknownTo.toLowerCase() ===
      '0x729f8f6a6970cfc7acae555236a20751c1501f39';
  const detailHeaderContent = isVortexTo
    ? 'Interaction with Vortex'
    : intl.formatMessage({
        id:
