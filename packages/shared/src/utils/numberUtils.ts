...
export const formatBalance: IFormatNumberFunc = (value, options) => {
  ...
  // Vortex fork — customer asked for at least 4-decimal precision on every
  // listed token balance (especially USDT, which used to render as "3.874"
  // instead of "3.8740"). For values >= 1 we now keep the trailing zeros so
  // the four decimal places are always visible. Sub-1 amounts already pad
  // past their leading zeros via the branch below, so trailing zeros there
  // are still trimmed to avoid runaway widths like "0.00010000".
  const fmtOpts = {
    digits: 4,
  ...
  if (absValue.gte(1)) {
    const fmtOptsKeepZeros = { ...fmtOpts, removeTrailingZeros: false };
    ...
    const {
      ...
      decimalSymbol,
      roundValue,
    } = formatLocalNumber(val, fmtOptsKeepZeros);
    return {
      formattedValue,
